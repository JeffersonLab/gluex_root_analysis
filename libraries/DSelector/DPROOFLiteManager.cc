...
bool DPROOFLiteManager::Process_Tree(string locInputFileName, string locTreeName, string locSelectorName, unsigned int locNumThreads, string locOutputFileName, string locOutputTreeFileName, string locFlatTreeFileName, string locOptions)
{
	TChain* locChain = new TChain(locTreeName.c_str());
	locChain->Add(locInputFileName.c_str());
	return Process_Chain(locChain, locSelectorName, locNumThreads, locOutputFileName, locOutputTreeFileName, locFlatTreeFileName, locOptions);
}

bool DPROOFLiteManager::Process_Chain(TChain* locChain, string locSelectorName, unsigned int locNumThreads, string locOutputFileName, string locOutputTreeFileName, string locFlatTreeFileName, string locOptions)
{
	string locPackageName = Get_PackagePath();
	Setup_PROOFSession(locPackageName, "", locOutputFileName, locOutputTreeFileName, locFlatTreeFileName, locOptions, locNumThreads);
	return Process_Chain(locChain, locSelectorName);
}

bool DPROOFLiteManager::Process_Other(string locSelectorName, string locInputFileName, unsigned int locNumThreads, unsigned int locNumEntries, string locOutputFileName, string locOutputTreeFileName, string locFlatTreeFileName, string locOptions)
{
	string locPackageName = Get_PackagePath();
	TProof* locPROOF = Setup_PROOFSession(locPackageName, locInputFileName, locOutputFileName, locOutputTreeFileName, locFlatTreeFileName, locOptions, locNumThreads);
...
TProof* DPROOFLiteManager::Setup_PROOFSession(string locPackageName, string locInputFileName, string locOutputFileName, string locOutputTreeFileName, string locFlatTreeFileName, string locOptions, unsigned int locNumThreads)
{
	...
	locFlatTreeFileName = Setup_Path(locFlatTreeFileName);
	...
	if(locFlatTreeFileName != "")
                locSessionVariables->AddLast(new TNamed("FLAT_TREE_FILENAME", locFlatTreeFileName.c_str()));
